...
class BinanceTrader(BaseCryptoTrader):
  """
  Concrete trader class for the Binance exchange (Spot).

  Authentication uses a standard API key + secret key pair.
  No passphrase is required. CCXT handles HMAC-SHA256 signing internally.
  ...
  the Binance dashboard and configure those credentials here. The
  subaccount_name field is a display-only label for the dashboard bot card.

  Binance.US users: set hostname to "api.binance.us" in the YAML config.
  """

  def __init__(self, config: dict, default_type: str = 'spot'):
    """
    Initializes the BinanceTrader for the Binance exchange.
    ...
    self.logger.success(f"BinanceTrader initialized for {self.account_identifier}")

  async def fetch_balance(self) -> Dict[str, Any]:
    """
    Fetches and logs the balance for the Binance account.
    """
    self.logger.info(f"Fetching balance for Binance account: {self.account_identifier}...")
  ...
  async def fetch_open_orders(self,
    ...
    """
    Fetches all open orders for the Binance account.
    ...
    if params is None:
      params = {}
    self.logger.info(
      f"Fetching open orders for {symbol} on Binance account: {self.account_identifier}..."
    )
  ...
  async def cancel_order(self,
    ...
    Args:
      ...
      symbol (str): Trading pair (required by Binance — must be provided).
      params (dict): Additional exchange-specific parameters.
    ...
    if params is None:
      params = {}
    self.logger.info(
      f"Cancelling order {order_id} on Binance account: {self.account_identifier}..."
    )
  ...
  async def list_fiat_markets(self, fiat_currency: str = "USDT") -> List[Dict[str, Any]]:
    """
    Fetches and lists all markets on Binance that involve fiat_currency.

    Args:
      fiat_currency (str): The fiat/quote currency to search for (e.g. 'USDT', 'USDC').
    """
    self.logger.info(
  ...
  async def convert_fiat_to_stablecoin(self,
    ...
    """
    Converts a percentage of available fiat/quote currency into a stablecoin.
  ...
  async def create_order(self,
    ...
    """
    Creates an order on Binance with flexible execution and amount.
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # LAYER 4 — EXECUTE ORDER  (Binance-specific)
    # For spend% market buys, use createMarketBuyOrderWithCost to pass the
    # exact quote cost. Binance natively supports quoteOrderQty via CCXT's
    # createMarketBuyOrderWithCost, so the fallback is rarely needed.
...
if __name__ == "__main__":
  # Test script for BinanceTrader - Run this file directly to test basic functionality.
  # Requires a configured bot_configs/crypto/binance.yaml file.
  ...
  async def test_binance_trader():
    """Test basic Binance trader functionality"""
    bots = [b for b in get_bot_configs() if b.get('exchange') == 'binance']
    if not bots:
      print("❌ No Binance bot configs found in bot_configs/crypto/binance.yaml")
      return

    config = bots[0]
    print(f"\n{'='*60}")
    print(f"Testing BinanceTrader with bot: {config['id']}")
    print(f"{'='*60}\n")
  ...
  asyncio.run(test_binance_trader())
